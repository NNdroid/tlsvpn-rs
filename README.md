...
The normal ARM64 artifacts remain generic and are the compatibility default. For Cortex-A55 systems such as RK3568-class devices, an additional tuned binary can be built without replacing the generic one:
...
Tagged releases also publish `tlsvpn-aarch64-unknown-linux-musl-cortex-a55` alongside the generic `aarch64-unknown-linux-musl` artifact. The tuned binary is compiled with `-C target-cpu=cortex-a55`; use it only on compatible CPUs.

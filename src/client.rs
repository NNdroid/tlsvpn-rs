...
fn dial_and_serve(cl: &Arc<Client>, conn_index: usize, ci: &Arc<ConnInfo>) -> Duration {
    ...
    *ci.last_error.lock() = String::new();
    ...
    while !conn_closed && !EXIT.load(Ordering::Relaxed) {
        ...
        // BackendNotify coalesces producer wakeups. A single wake may therefore
        // represent much more than one 32 KiB TLS plaintext batch. After consuming
        // that wake, keep servicing already-queued frames without sleeping for the
        // RTT/reorder timer. If rustls still has ciphertext blocked on the socket,
        // preserve the normal timeout instead of spinning.
